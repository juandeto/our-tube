...
class VideoListController {
  constructor(videoListService = new video_list_service_1.VideoListService()) {
    ...
    this.createVideoList = (req, res) =>
      __awaiter(this, void 0, void 0, function* () {
        ...
        const { error: errorService, result } =
          yield this.videoListService.createVideoList(createTodoDto);
        if (error) {
          return res.status(400).json({ errorService });
        }
        return res.status(201).json(result);
      });
    this.updateVideoList = (req, res) =>
      __awaiter(this, void 0, void 0, function* () {
        const id = req.params.id;
        const [error, updateVideoList] =
          update_videoList_1.UpdateVideoListDto.update(req.body);
        if (error || !updateVideoList) {
          return res.status(404).json({ error });
        }
        const { error: errorService, result } =
          yield this.videoListService.updateVideoList(id, updateVideoList);
        if (error) {
          return res.status(400).json({ errorService });
        }
    ...
    this.deleteVideoList = (req, res) =>
      __awaiter(this, void 0, void 0, function* () {
        const id = req.params.id;
        if (!id) {
          res.json({ error: 'invalid id: ' + id });
        }
        const { error, result } = yield this.videoListService.deleteVideoList(
          id
        );
        if (!error) {
          return res
